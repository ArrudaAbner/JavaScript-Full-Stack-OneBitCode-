...
function addSpaceship(name, pilot, crewLimit) {
    ...
    alert(`Nave ${name} pilotado pelo ${pilot} com tripulação máxima de ${crewLimit} pessoas foi criada!`);
}
...
function addCrewMember(member, spaceship) {
    if (spaceship.crew.length >= spaceship.crewLimit) {
        ...
        alert(`O ${member} foi adicionado a tripulação da nava ${spaceship.name}!`);
    }
}
function sendInMission(spaceship) {
    if (spaceship.inMission) {
        alert(`A nave ${spaceship.name} não pode ser enviada! Nave já em missão`);
    }
...
function firstMenuOption() {
    ...
    const crewLimit = Number(prompt("Quantos tripulantes a nave tem suporta?"));
